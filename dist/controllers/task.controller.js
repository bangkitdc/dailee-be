...
class TaskController {
    ...
    async addOrUpdateTask(req, res) {
        ...
        return response_helper_1.ResponseHelper.responseSuccess(res, http_enum_1.HttpStatusCode.Ok, 'Task submitted successfully', updatedTask);
    }
    async getTask(req, res) {
        const { date } = req.params;
        const tasks = await this.taskService.getTask(req.user.user_id, date);
        return response_helper_1.ResponseHelper.responseSuccess(res, http_enum_1.HttpStatusCode.Ok, 'Operation successful', tasks);
    }
    async checkTask(req, res) {
        const { task_id } = req.params;
        await this.taskService.checkTask(req.user.user_id, parseInt(task_id));
        return response_helper_1.ResponseHelper.responseSuccess(res, http_enum_1.HttpStatusCode.Ok, 'Task checked off successfully');
    }
    async getCompletedTasks(req, res) {
        const completedTasks = await this.taskService.getCompletedTask(req.user.user_id);
        return response_helper_1.ResponseHelper.responseSuccess(res, http_enum_1.HttpStatusCode.Ok, 'Operation successful', completedTasks);
    }
}
